...
Image::Image(const Image &old_obj) : image(nullptr), width(0), height(0), filename(old_obj.filename) {
  allocateImage(old_obj.width,old_obj.height);
  for(unsigned int i = 0; i < width; ++i) {
    for(unsigned int j = 0; j < height; ++j) {
      image[i][j] = old_obj.image[i][j];
    }
  }
}
...
Image& Image::operator=(const Image &source) {
  ...
  // std::swap(filename, temp.filename);
