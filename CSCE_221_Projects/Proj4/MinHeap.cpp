...
MinHeap<T, m_size>::MinHeap(const Heap<T, m_size> &origHeap) {
    ...
    buildHeap();
    // Gets rid of nonexistent value bug
    Remove();
}

template <class T, int m_size>
bool MinHeap<T, m_size>::Contains(const T &needle) const {
    if(currentSize == 0) {
...
template <class T, int m_size>
const T *MinHeap<T, m_size>::Find(const T &needle) const {
    T *found;
...
template <class T, int m_size>
void MinHeap<T, m_size>::PercolateUp(int index, T& insertable) {
...
template <class T, int m_size>
void MinHeap<T, m_size>::buildHeap() {
